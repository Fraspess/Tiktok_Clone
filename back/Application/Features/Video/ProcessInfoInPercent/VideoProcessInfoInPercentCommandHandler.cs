...
{
    internal class VideoProcessInfoInPercentCommandHandler(IUnitOfWork _uow, IVideoProcessingNotifier videoProcessingNotifier) : IRequestHandler<VideoProcessInfoInPercentCommand, Unit>
    {
        public async Task<Unit> Handle(VideoProcessInfoInPercentCommand request, CancellationToken cancellationToken)
        {
            ...
            await videoProcessingNotifier.SendVideoProcessingProgress(request.VideoId, video.UserId, request.Percentage);
