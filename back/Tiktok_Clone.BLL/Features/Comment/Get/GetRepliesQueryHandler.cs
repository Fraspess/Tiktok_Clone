using MediatR;
using Tiktok_Clone.BLL.Dtos.Comment;
using Tiktok_Clone.BLL.Features.Comment.GetReplies;
using Tiktok_Clone.BLL.Pagination;
using Tiktok_Clone.BLL.Services.Comment;

namespace Tiktok_Clone.BLL.Features.Comment.Get
{
    public class GetRepliesQueryHandler(ICommentService service) : IRequestHandler<GetRepliesQuery, PagedResult<CommentDTO>>
    {
        public async Task<PagedResult<CommentDTO>> Handle(GetRepliesQuery request, CancellationToken cancellationToken)
        {
            return await service.GetRepliesAsync(request.ParentCommentId, request.PaginationSettings);
        }
    }
}
